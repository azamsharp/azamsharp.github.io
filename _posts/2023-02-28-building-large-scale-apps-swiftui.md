...
> In most client/server apps, where the client (SwiftUI) is simply displaying data on the view and interacting with the server using POST requests the above pattern is all you need. The actual business logic is hosted on the server and the client is simply displaying the data and performing presentation logic on the displayed data (sorting, filtering etc).  

...
The above application works as expected and produces the expected result. Instead of testing the concrete implementation of the ```Webservice``` against a TEST server, we introduce an interface/contract/protocol just so we can inject a mock. The sole purpose of creating the protocol is so we can satisfy the tests, even though their is only one concrete implementation that conforms to that procotol/interface.  

>> This is called [**Test Induced Damage**](https://dhh.dk/2014/test-induced-design-damage.html). The tests are dictating that we should add dependencies so you can mock out the service. The only purpose of introducing a protocol/contract/interface is so you can eventually mock it. Keep in mind there is nothing wrong with using protocols/contracts in your application. They do serve a very important purpose to hide the implementation details from the user. But just to add contracts to satisfy testing goals in not a good practice as it complicates the implementation and your tests are directed away from testing the behavior of the app. 
...
Some frameworks also allow you to construct in-memory databases. Core Data for instance uses SQLite by default but it can be configured to use an in-memory database as shown below: 
...
We talked a lot about domain level unit tests, integration tests and E2E tests (UI Tests). At this point, you may be wondering what is the best kind of test to write. What is the ideal test? 
...
Remember to test the public API exposed by the module and not the implementation details. This way you will write useful quality tests, which will also help you to catch errors.  

Don't create interfaces/contracts with the sole purpose of mocking. If a interface/contract consists of a single concrete implementation then use the concrete and remove the interface/contract. Your architecture should be based on current business needs and not on what if scenarios that may never happen. Remember YAGNI (You aren't going to need it). Less code is much better than more code. 

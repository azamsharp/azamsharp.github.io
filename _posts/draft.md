...
I have been working in the iOS ecosystem for a long time and one thing I have consistently observed in our community is a reluctance to explore technologies outside the Apple ecosystem. If a framework is not written in Swift, many iOS developers run away faster than a speeding bullet. While it is great to be productive with Apple tools, limiting yourself to a single ecosystem can also limit how you think about software.

Learning different technologies makes you a better engineer. Even if you never end up using a particular framework in production, the experience of learning it expands how you think about architecture, systems, and problem solving. So in this series we will step slightly outside the traditional iOS comfort zone and build parts of the system using technologies that many iOS developers rarely touch.
...
First make sure **Node.js** is installed on your machine. There are several ways to install Node. You can use Homebrew, download the installer from the official Node website, or use a version manager such as nvm.

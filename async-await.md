...
Asynchronous programming using async/await is  

---

### Asynchronous Programming Using Callbacks 

Before we jump into the new async/await features in iOS 15, we need to see how things are handled currently. It is by looking at the the paste, we can appreciate the future. 

For this example we are going to use [JSONPlacefolder API](https://jsonplaceholder.typicode.com/posts) and fetch all posts. The implementation is shown below: 
...
The main issue with the above code is that you need to remember to return from the guard clauses. The other issue is the overall length of the code. For a simple operation like this, it turns out to be 10-15 lines of code. 

In the next section, we are going to implement the same code but using the new async/await features. 

### Understanding Async/await 

Async and await is not something new, several different programming languages including C#, and JavaScript have been using async/await for a very long time.
...
The async/await version is not only fewer lines of code, but it is also much easier to understand. The function `getAllPosts` is also decorated with throws, which indicate that the awaiting function can throw an error. This is the reason we called the `URLSession.shared.data` with `try await` instead of just await. Also note that you can only decorate the function with await, if it is marked with async.   
...
The `@MainActor` is part of the new actor API introduced in Swift language. The `@MainActor` makes sure that all properties and functions in PostListViewModel are called on the main thread. 
...
One thing you will notice is the use of `task` modifier. The task modifier is introduced in iOS 15 and it used to perform a task when the view is loaded. The task modifier is cancelled when the view disappears. 

> At this point you might be wondering that what will happen to onAppear function of the view. According to the conversations with Apple engineers, currently there are no plans to depreciate onAppear.    
...
The problem is that we are using await to call the fetchPosts function, since we will have to wait for the results. But await cannot be used unless the function is async. But how can we decorate the button click function with async? The answer is to use async closures. This is shown below: 
...
Async and await provides a great way to perform asynchronous tasks but what about your legacy callbacks implementation. You can use the new continuation feature in iOS 15 to invoke your callbacks using async/await. 
...
You can see we are wrapping the call to `getAllPosts` inside the `withCheckedThrowingContinuation` closure. This means that once your getAllPosts call completes, continuation can either resume or throw an error. 

Write something about different continuation functions available in iOS 15. 
